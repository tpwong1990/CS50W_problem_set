function page_navigator(current_page, category_id){
    ...
    if (current_page == 1) {
        ...
        previous_button.innerHTML = `<a class="page-link" href="javascript:load_grid_items(${current_page - 1}, ${category_id})" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>`;
    };
    ...
    if (current_page == total) {
        ...
        next_button.innerHTML =`<a class="page-link" href="javascript:load_grid_items(${current_page + 1}, ${category_id})" aria-label="Next"><span aria-hidden="true">&raquo;</span></a>`;
    };
    ...
    if (total <= 7) {
        for (var i = 1; i <= total; i++) {
            ...
            if ( i == current_page) {
                ...
                li.innerHTML = `<a class="page-link" href="javascript:load_grid_items(${i}, ${category_id})">${i}</a>`;
            };
        };
        // Remove exact button
        //console.log(`t:${total}`);
        ...
        if (current_page <= 3) {
            ...
            li.setAttribute('class', "page-item");
            li.removeAttribute("aria-current");
            li.innerHTML = `<a class="page-link" href="javascript:load_grid_items(${total}, ${category_id})">${total}</a>`
        };
        ...
        if (current_page > 3 && current_page < total - 3) {
            ...
            li.setAttribute('class', "page-item");
            li.removeAttribute("aria-current");
            li.innerHTML = `<a class="page-link" href="javascript:load_grid_items(1, ${category_id})">1</a>`;
            ...
            for (var i = 3; i <= 5; i++) {
                ...
                if ( i == 4) {
                    ...
                    li.removeAttribute("aria-current");
                    if (i == 3) {
                        li.innerHTML = `<a class="page-link" href="javascript:load_grid_items(${current_page-1}, ${category_id})">${current_page-1}</a>`;
                    };
                    if (i == 5) {
                        li.innerHTML = `<a class="page-link" href="javascript:load_grid_items(${current_page+1}, ${category_id})">${current_page+1}</a>`;    
                    }
        ...
        if (current_page >= total - 3){
            ...
            for (var i = 3; i <= 7; i++) {
                ...
                if (current_page == total-(7-i)) {
                    ...
                    li.setAttribute('class', "page-item");
                    li.removeAttribute("aria-current");
                    li.innerHTML = `<a class="page-link" href="javascript:load_grid_items(${total-(7-i)}, ${category_id})">${total-(7-i)}</a>`;
                };
            };
